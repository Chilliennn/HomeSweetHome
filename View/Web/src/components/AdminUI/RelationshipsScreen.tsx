...
export const RelationshipsScreen: React.FC<Props> = ({ onNavigate }) => {
    ...
    const calculateLastContact = (lastMessageAt: string | null): string => {
    ...
    const determineStatus = (rel: DBRelationship): 'healthy' | 'caution' | 'critical' => {
            ...
            return (
                <>
                    ...
                    <section style={{
                        ...
                        <RelationshipStatCard value={relationships.filter(r => r.status === 'healthy' || r.status === 'caution' || r.status === 'critical').length} label="Total Active Families" color="#9DE2D0" />
                        <RelationshipStatCard value={relationships.filter(r => r.status === 'healthy').length} label="Healthy Relationships" color="#9DE2D0" />
                        <RelationshipStatCard value={relationships.filter(r => r.status === 'critical').length} label="Needs Attention" color="#EB8F80" highlighted />
                    </section>
